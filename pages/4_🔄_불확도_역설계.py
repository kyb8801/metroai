...
st.caption("🌟 세계 최초 기능 — MetroAI만의 차별점")

# ──────────────────────────────────────────
# KOLAS 컨텍스트 가이드 (NEW — v0.5.0)
# ──────────────────────────────────────────
with st.expander("💡 **불확도 역설계란? — 처음이신 분 클릭**", expanded=False):
    st.markdown(
        """
        **보통의 불확도 계산 (정방향):**
        > 측정 조건 (반복성, 표준기 불확도, 환경 불확도 등) → **합성불확도 계산**

        **역설계 (역방향):**
        > 목표 불확도 (CMC) → **"각 요소를 얼마까지 허용할 수 있나?"** 역으로 산출

        ---

        **📌 구체적 예시:**

        *"우리 기관의 블록게이지 교정능력(CMC)을 **0.1 μm** 으로 KOLAS에 신고하고 싶다면..."*
        - 반복성은 **0.06 μm 이하**여야 하고
        - 표준기 불확도는 **0.05 μm 이하**여야 하고
        - 온도 관리는 **±0.3°C 이내**여야 한다

        → 이걸 MetroAI가 자동으로 계산해줍니다.

        ---

        **📌 이런 분에게 유용합니다:**
        - **새로운 교정 품목을 추가하려는 기관** — "이 CMC를 달성하려면 어떤 장비가 필요한가?"
        - **CMC를 개선하고 싶은 기관** — "어느 요소를 줄여야 가장 효과적인가?"
        - **장비 업그레이드 의사결정** — "표준기를 바꾸면 CMC가 얼마나 개선되는가?"
        - **불확도 계산에서 기본값을 사용한 경우** — "그 기본값이 결과에 얼마나 영향을 미치는가?"
        """
    )
