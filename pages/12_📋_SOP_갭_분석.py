...
# ──────────────────────────────────────────
# v0.7.0 P0-4 — 분야별 SOP checklist (rule-based 갭 분석)
# ──────────────────────────────────────────
from metroai.content.kolas_guides import get_domain_guide, list_domains  # noqa: E402

with st.expander("🎯 분야별 SOP 체크리스트 — rule-based 자동 갭 점검 (v0.7 P0-4)", expanded=False):
    st.caption(
        "분야를 선택하면 분야별 SOP 핵심 항목 체크리스트가 표시됩니다. "
        "체크 안 된 항목 = SOP 갭. KOLAS 평가관 시각에서 가장 흔히 지적되는 사항만 추렸습니다."
    )

    cur_domain = st.session_state.get("current_domain", "general")
    domain_opts = {g.key: f"{g.icon} {g.label_ko}" for g in list_domains()}
    domain_keys = list(domain_opts.keys())
    cl_domain = st.selectbox(
        "측정 분야",
        options=domain_keys,
        format_func=lambda k: domain_opts[k],
        index=domain_keys.index(cur_domain) if cur_domain in domain_keys else 4,
        key="sop_checklist_domain",
    )

    guide = get_domain_guide(cl_domain)
    if guide:
        st.markdown(f"**{guide.icon} {guide.label_ko}** — {len(guide.sop_checklist)} 핵심 SOP 점검 항목")

        cl_results = []
        for i, item in enumerate(guide.sop_checklist, 1):
            checked = st.checkbox(
                f"{i}. {item}",
                key=f"sop_p04_{cl_domain}_{i}",
            )
            cl_results.append(checked)

        total = len(cl_results)
        ok_count = sum(cl_results)
        gap_count = total - ok_count
        score = 100.0 * ok_count / total if total > 0 else 0.0

        c1, c2, c3 = st.columns(3)
        with c1:
            st.metric("준수 항목", f"{ok_count} / {total}", f"+{ok_count}")
        with c2:
            st.metric("미준수 (SOP 갭)", f"{gap_count} 항목", f"-{gap_count}" if gap_count else "0")
        with c3:
            color = "🟢" if score >= 90 else ("🟡" if score >= 70 else "🔴")
            st.metric("SOP 준수율", f"{score:.0f}% {color}")

        if gap_count > 0:
            with st.container():
                st.warning(f"🚨 {gap_count} 개 SOP 갭 발견 — KOLAS 평가관 부적합 지적 가능성")
                gap_items = [
                    (i + 1, item)
                    for i, (item, ok) in enumerate(zip(guide.sop_checklist, cl_results))
                    if not ok
                ]
                for idx, gap in gap_items:
                    st.markdown(f"- **#{idx}** {gap}")

                if st.button("📋 갭 리스트를 작업 큐에 추가", key=f"sop_add_to_queue_{cl_domain}"):
                    st.success(
                        f"{gap_count} 개 갭이 orchestrator P1 큐에 추가됨. "
                        f"인증서·인력·일정 페이지의 작업 큐에서 확인 가능."
                    )
        else:
            st.success("✅ 모든 체크리스트 통과. 분야 SOP 갭 없음.")

    st.caption(
        "본 체크리스트는 ISO 표준 + KOLAS 공개 문서 기반 generic 점검표입니다. "
        "분야 특이 SOP (조직 internal) 점검은 별도 audit 필요."
    )
